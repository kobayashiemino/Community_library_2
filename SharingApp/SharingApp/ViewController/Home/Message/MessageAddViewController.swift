...
//        present(imagePickerController, animated: true, completion: nil)
    ...
    @objc private func didTapSubmitButton() {
        ...
        if let image = photoImageView.image {
            ...
            StorageManeger.shared.uploadPostPhoto(with: imageData, fileName: fileName) { result in
                switch result {
                case .success(let url):
                    `self`.saveToDatabase(urlString: url, urlStrings: nil)
            ...
            StorageManeger.shared.uploadPhotos(with: imageDatas, fileName: fileName) { result in
                switch result {
                case .success(let urls):
                    guard let urls = urls as? [String] else { return }
                    `self`.saveToDatabase(urlString: nil, urlStrings: urls)
                case .failure(let error):
                    print("failed to upload Data:\(error)")
                }
            }
    ...
    private func saveToDatabase(urlString: String?, urlStrings: [String]?) {
        ...
        if let urlString = urlString {
            uploadData.append(urlString)
        } else if let urlStrings = urlStrings {
            uploadData.append(contentsOf: urlStrings)
        }
...
extension MessageAddViewController: SelectRankPopupDelegate {
...
extension MessageAddViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension MessageAddViewController: ImagePickerDelegate {
...
extension MessageAddViewController: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        pageControl.currentPage = Int(floorf(Float(scrollView.contentOffset.x / scrollView.width)))
    }
}

extension MessageAddViewController: SDGsViewDelegate {
